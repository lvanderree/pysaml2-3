...
class ExtensionElement(object):
    ...
    def loadd(self, ava):
        ...
        if "attributes" in ava:
            for key, val in list(ava["attributes"].items()):
...
def _extension_element_from_element_tree(element_tree):
    ...
    for key, value in list(element_tree.attrib.items()):
...
class ExtensionContainer(object):
    ...
    def harvest_element_tree(self, tree):
        ...
        for attribute, value in list(tree.attrib.items()):
    ...
    def _add_members_to_element_tree(self, tree):
        ...
        for attribute, value in list(self.extension_attributes.items()):
...
class SamlBase(ExtensionContainer):
    ...
    def _get_all_c_children_with_order(self):
        if len(self.c_child_order) > 0:
            ...
            for _, values in list(self.__class__.c_children.items()):
                yield values[0]
    ...
    def _add_members_to_element_tree(self, tree):
        ...
        for xml_attribute, attribute_info in \
                list(self.__class__.c_attributes.items()):
    ...
    def keyswv(self):
        ...
        :return: list of keys
        """
        return [key for key, val in list(self.__dict__.items()) if val]
    ...
    def keys(self):
        ...
        """
        keys = ['text']
        keys.extend([n for (n, t, r) in list(self.c_attributes.values())])
        keys.extend([v[0] for v in list(self.c_children.values())])
        return keys
    ...
    def loadd(self, ava, base64encode=False):
        ...
        for prop, _typ, _req in list(self.c_attributes.values()):
            #print "# %s" % (prop)
            if prop in ava:
        ...
        for prop, klassdef in list(self.c_children.values()):
    ...
    def __eq__(self, other):
        ...
        for key in self.keyswv():
            ...
            if isinstance(svals, str):
                if not isinstance(ovals, str):
                    return False
                if svals != ovals:
                    return False
            elif isinstance(svals, list):
                if not isinstance(ovals, list):
                    if len(svals) != 1:
                        return False
                    elif ovals == svals[0]:
                        pass
...
def element_to_extension_element(element):
    ...
    for xml_attribute, (member_name, typ, req) in \
            list(element.c_attributes.items()):
