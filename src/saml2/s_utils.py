...
def decode_base64_and_inflate(string):
    ...
    return (zlib.decompress(base64.b64decode(string), -15)).decode("utf8")
...
def deflate_and_base64_encode(string_val):
    ...
    """
    _uc = zlib.compress(string_val.encode("utf8"))
    return base64.b64encode(_uc[2:-4])


def rndbytes(size=16):
    """
    ...
    _txt = "".join([random.choice(_basech) for _ in range(size)])
    return _txt.encode("utf8")
...
def sid(seed=""):
    ...
    """
    ident = md5()
    ident.update(repr(time.time()).encode("utf8"))
    if seed:
        try:
            ident.update(seed.encode("utf8"))
...
def signature(secret, parts):
    ...
    try:
        ...
        csum = hmac.new(secret.encode("utf8"), digestmod=hashlib.sha1)

    for part in parts:
        try:
            csum.update(part)
        except TypeError:
            csum.update(part.encode("utf8"))
