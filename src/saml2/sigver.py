...
def signed_instance_factory(instance, seccont, elements_to_sign=None):
    ...
    if elements_to_sign:
        ...
        for (node_name, nodeid) in elements_to_sign:
            signed_xml = rm_xmltag(signed_xml)
...
def parse_xmlsec_output(output):
    ...
    :param output: The output from Popen {bytes}
    :return: A boolean; True if the command was a success otherwise False
    """
    _txt = output.decode("utf8")
...
class CryptoBackend():
    ...
    def encrypt(self, text, recv_key, template, key_type, xpath):
        raise NotImplementedError()
